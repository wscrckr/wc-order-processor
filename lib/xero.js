...
xero.getInvoices = function getInvoices(whereFilter, orderBy, pageNum, cb) {
  ...
  getHelper(xero.url.invoices, qs, cb);
}
...
xero.getContacts = function getContacts(whereFilter, cb) {
  ...
  getHelper(xero.url.contacts, qs, cb);
}

// POST a new contact
xero.addContact = function addContact(jsonPayload, rootTag, cb) {
  postHelper(xero.url.contacts, jsonPayload, rootTag, cb);
}
...
// Helper function to make a POST call
var postHelper = function postHelper(url, jsonPayload, rootTag, cb) {
  ...
  rp.post(options)
    .then(cb)
    .catch(function (err) {
      console.err('Xero API POST failed!');
      console.error(err);
    });
}
